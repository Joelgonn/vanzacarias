-- Copia para execução manual no Supabase Dashboard > SQL Editor
-- Mesma definição de supabase/migrations/20250901_create_commerce_events.sql
...
create policy "service_role_all" on public.commerce_events for all using (auth.role() = 'service_role') with check (auth.role() = 'service_role');
