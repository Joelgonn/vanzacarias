  ...
  const load = async (): Promise<void> => {
    ...
    loadPromise = (async () => {
      try {
        ...
        // KO-007.0: Android com server.url remoto usa Vercel assets (public/assets/tts servido em https://vanzacarias-mu.vercel.app/assets/tts/)
        const managerOrigin = (modelManager as unknown as { getOrigin?: () => string })?.getOrigin?.() ?? ""
        const effectiveBaseUrl =
          brow?.baseUrl ??
          (native
            ? "https://vanzacarias-mu.vercel.app/assets/tts/"
            : managerOrigin
